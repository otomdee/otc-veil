...
export default function SelectWallet({ variant = "ctaBig" }: { variant?: "nav" | "ctaBig" }) {
  ...
  useEffect(() => {
    ...
    // The discovery package bundles its own copy of wallet-standard types; cast
    // across the duplicate declarations.
    setWallets(store.getWallets().slice() as unknown as WalletWithStarknetFeatures[]);
    const unsub = store.subscribe(
      (next) => setWallets(next.slice() as unknown as WalletWithStarknetFeatures[])
    );
    return () => unsub();
  }, []);
